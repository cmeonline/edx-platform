...
OpenStax Integrator Module. Check out the project's [documentation](http://queriumCorp.github.io/openstax_integrator/).
...
```
heroku create openstax_integrator-prod --remote prod && \
    heroku addons:create newrelic:wayne --app openstax_integrator-prod && \
    heroku addons:create heroku-postgresql:hobby-dev --app openstax_integrator-prod && \
    heroku config:set DJANGO_SECRET=`openssl rand -base64 32` \
        DJANGO_AWS_ACCESS_KEY_ID="Add your id" \
        DJANGO_AWS_SECRET_ACCESS_KEY="Add your key" \
        DJANGO_AWS_STORAGE_BUCKET_NAME="openstax_integrator-prod" \
        --app openstax_integrator-prod
```

Creating the qa sever:

```
heroku create `openstax_integrator-qa --remote qa && \
    heroku addons:create newrelic:wayne && \
    heroku addons:create heroku-postgresql:hobby-dev && \
    heroku config:set DJANGO_SECRET=`openssl rand -base64 32` \
        DJANGO_AWS_ACCESS_KEY_ID="Add your id" \
        DJANGO_AWS_SECRET_ACCESS_KEY="Add your key" \
        DJANGO_AWS_STORAGE_BUCKET_NAME="openstax_integrator-qa" \
```
